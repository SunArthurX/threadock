          ...
          <div className="ops-risky">
            {anomalyPager.slice.map((a, i) => (
              <div key={i} className="ops-risky-row">
                ...
                {a.source_session_id && (
                  <button className="finding-btn" title={t("跳转到对应会话")}
                    onClick={() => p.onJump(a.provider ?? "*", a.source_session_id!, null)}>{t("→ 会话")}</button>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="ops-card">
        ...
        {p.audit == null && !p.auditing && (
          <div className="audit-hero">
            ...
            <div className="audit-hero-body">
              <div className="audit-hero-title">{t("首次访问？点此开始全库扫描")}</div>
              <div className="audit-hero-sub">
                {t("扫描器会遍历本地所有已归档会话，识别敏感信息（密钥、token、邮箱）")}
                {t("与危险命令（rm -rf、sudo、chmod 777 等），并按正则规则匹配。")}
                {t("通常 1–2 分钟内完成；扫描结果保存到本地，不会上传到任何服务器。")}
              </div>
              <div className="audit-hero-cta">
                <button className="action-btn primary" onClick={p.onScan}>{t("▶ 开始全库扫描")}</button>
              </div>
            </div>
          </div>
        )}
        <div className="audit-toolbar">
          ...
          <button className={`action-btn ${p.audit == null && !p.auditing ? "" : "primary"}`} onClick={p.onScan}>
            {p.auditing ? t("扫描中…") : p.audit ? t("↻ 重新扫描") : t("▶ 开始全库扫描")}
          </button>
          {p.audit && p.audit.findings.length > 0 && (<>
            <button className="action-btn" onClick={p.onExportHtml}>{t("⤓ 导出报告")}</button>
            <button className="action-btn" onClick={() => handleBulk("ignored")} title={t("全部标记为「忽略」（本会话不再报）")}>{t("⊘ 全部忽略")}</button>
            <button className="action-btn" onClick={() => handleBulk("false_positive")} title={t("全部标记为「误报」（同类规则不再报）")}>{t("⊗ 全部误报")}</button>
            <div className="ops-range">
              {([["all",t("全部")],["sensitive",t("敏感信息")],["dangerous_command",t("危险命令")]] as const).map(([v,l]) => (
            ...
            {findings.slice(0, 50).map((f: AuditFinding, i) => (
              <div key={i} className="audit-finding-row" onClick={() => p.onJump(f.provider, f.source_conversation_id, f.message_id)}>
                ...
                <span className="finding-actions" onClick={(e) => e.stopPropagation()}>
                  <button className="finding-btn" title={t("不再提示此发现")} onClick={() => { p.onDisposeFinding(f.fingerprint, "ignored"); p.onRefreshAfterDispose(); }}>{t("忽略")}</button>
                  <button className="finding-btn" title={t("标记为误报（同类不再报）")} onClick={() => { p.onDisposeFinding(f.fingerprint, "false_positive"); p.onRefreshAfterDispose(); }}>{t("误报")}</button>
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="policy-section">
          <div className="section-subhead">
            ...
            {t("自定义策略规则（正则）")}
            <button className="kb-copy" style={{ marginLeft: "auto" }} onClick={exportPolicies} title={t("把当前规则复制为 JSON 粘贴到剪贴板")}>{t("导出")}</button>
            <button className="kb-copy" onClick={importPolicies} title={t("粘贴 JSON 批量导入（同名规则覆盖）")}>{t("导入")}</button>
          </div>
          <div className="policy-add">
            <input placeholder={t("规则名")} value={p.newPolicy.name} onChange={(e) => p.onPolicyInput("name", e.target.value)} />
            <input placeholder={t("正则")} value={p.newPolicy.pattern} onChange={(e) => p.onPolicyInput("pattern", e.target.value)} />
            <select value={p.newPolicy.kind} onChange={(e) => p.onPolicyInput("kind", e.target.value)}>
              <option value="dangerous_command">{t("危险命令")}</option>
              <option value="sensitive">{t("敏感信息")}</option>
            </select>
            <select value={p.newPolicy.severity} onChange={(e) => p.onPolicyInput("severity", e.target.value)}>
              <option value="high">{t("高危")}</option><option value="medium">{t("中危")}</option><option value="low">{t("低危")}</option>
            </select>
            <button className="action-btn" onClick={p.onAddPolicy}>{t("＋ 添加")}</button>
          </div>
          {p.policies.length > 0 && (
            ...
            return (
              <div key={r.id} className={`ops-risky-item ${open ? "open" : ""}`}>
                <div className="ops-risky-row" onClick={() => p.onToggleRisk(r.id)}>
                  ...
                  {r.destructive && <span className="risk-flag high">{t("危险")}</span>}
                ...
                {open && (
                  <div className="ops-risky-detail">
                    <div className="risk-detail-grid">
                      <div><b>{t("时间：")}</b>{new Date(r.ts_ms).toLocaleString("zh-CN")}</div>
                      <div><b>{t("状态：")}</b>{r.status}</div>
                      <div><b>{t("退出码：")}</b>{r.exit_code ?? "—"}</div>
                      <div><b>{t("耗时：")}</b>{r.duration_ms != null ? formatDuration(r.duration_ms) : "—"}</div>
                    </div>
                    ...
                    <button className="action-btn" onClick={() => p.onJump(r.provider, r.source_session_id, null)}>{t("→ 跳转到对应会话")}</button>
                  </div>
                )}
              </div>
            );
