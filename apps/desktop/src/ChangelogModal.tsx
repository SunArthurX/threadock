    ...
    <div className="settings-backdrop" onClick={close}>
      <div className="settings-modal changelog-modal" onClick={(e) => e.stopPropagation()}>
        <div className="settings-header">
          <h2><Icon name="sparkle" size={16} /> 已更新到 v{APP_VERSION}</h2>
          <button className="settings-close" onClick={close}><Icon name="close" size={14} /></button>
        </div>
        <ScrollArea className="settings-body changelog-body">
          ...
          <ul className="changelog-list">
            {cur.highlights.map((h, i) => (
              <li key={i}>
                <span className="changelog-bullet" aria-hidden><Icon name="check" size={11} /></span>
                <span>{h}</span>
              </li>
            ))}
          </ul>
