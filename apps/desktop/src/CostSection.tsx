...
export default function CostSection({
  ...
  const budgetCard = (
      <div className="ops-card ops-budget">
        <CardTitle icon="dollar" sub={t("按当前速率自动检测超支并提醒")}>{t("月度预算")}</CardTitle>
        <div className="budget-grid">
          {/* 左侧两个大数字 — 进度条 + 数字突出 */}
          <div className="budget-stat">
            <div className="budget-stat-label">{t("本月 Tokens")}</div>
            ...
            {tokenPct != null ? (
              ...
              <div className="budget-stat-hint">{t("未设置上限")}</div>
            )}
          </div>
          <div className="budget-stat">
            <div className="budget-stat-label">{t("本月成本")}</div>
            <div className="budget-stat-value">
          ...
          <div className="budget-form">
            <div className="budget-form-field">
              <label className="budget-form-label" htmlFor="budget-tokens">{t("Token 上限")}</label>
              <input id="budget-tokens" className="budget-form-input" type="text" inputMode="numeric"
                placeholder={t("例如 5000000")} value={budgetInput.tokens}
                onChange={(e) => onBudgetInput("tokens", e.target.value)} />
            </div>
            <div className="budget-form-field">
              <label className="budget-form-label" htmlFor="budget-cost">{t("成本上限（USD）")}</label>
              <input id="budget-cost" className="budget-form-input" type="text" inputMode="decimal"
                placeholder={t("例如 50")} value={budgetInput.cost}
  ...
  return (
    <>
          ...
          <div className="projection-grid">
            {projTokenOver != null && (
              <div className="projection-item">
                <span className="projection-label">{t("预测 Tokens")}</span>
            ...
            {projCostOver != null && (
              <div className="projection-item">
                <span className="projection-label">{t("预测成本")}</span>
      ...
      <div className="ops-card">
        <CardTitle icon="folder" trailing={<button className="action-btn" onClick={onRecalc}>{t("重算")}</button>}>{t("按项目成本 Top10")}</CardTitle>
        {dirCosts.length === 0 ? (
          loading ? <Skeleton variant="list" count={5} /> : <InlineEmpty message={t("暂无项目成本数据")} hint={t("导入并使用 Agent 后将按 source_dir 自动归并")} />
        ) : (
          <table className="ops-table">
            <thead><tr><th>{t("项目目录")}</th><th>Tokens</th><th>{t("成本")}</th><th>{t("请求")}</th>{onJumpByDir && <th></th>}</tr></thead>
            <tbody>
              {dirCosts.map((d, i) => (
      ...
      {byProvider && byProvider.length > 0 && (
        <div className="ops-card">
          <CardTitle icon="globe">{t("按 Agent（Provider）成本分布")}</CardTitle>
          <BarChart
      ...
      {byModel && byModel.length > 0 && (
        <div className="ops-card">
          <CardTitle icon="cpu">{t("按模型成本 Top10")}</CardTitle>
          <div className="ops-table-wrap">
            <table className="ops-table">
              <thead><tr><th>{t("模型")}</th><th>Provider</th><th>{t("成本")}</th><th>Tokens</th><th>{t("请求")}</th><th>{t("错误")}</th>{onJumpByModel && <th></th>}</tr></thead>
              <tbody>
      ...
      {wow && (
        <div className="ops-card">
          ...
          <div className="wow-grid">
            ...
            <div className="wow-col">
              <div className="wow-label">{t("上周")}</div>
            ...
            <div className="wow-col wow-delta">
              <div className="wow-label">{t("变化")}</div>
