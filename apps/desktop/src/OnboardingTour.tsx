...
const STEPS_EMPTY = (): Step[] => [
  {
    icon: "import",
    title: t("导入你的 AI 工具对话"),
    body: t("点左上角的「同步」按钮，把 Cursor / Claude Code / ZCode / Codex 里的历史对话拉进 Threadock，统一管理。"),
    hint: t("导入完成后左侧会出现「来源筛选」chip，可以按工具单独看。"),
  },
  {
    icon: "command",
    title: t("命令面板：⌘K 唤起"),
    body: t("任意页面按 ⌘K（macOS）/ Ctrl+K（其它）唤起命令面板：搜索会话、按关键字跳页、运行操作。"),
    hint: t("试试输入「成本」「活动」会直接跳到对应页。"),
  },
  {
    icon: "compass",
    title: t("八个视图各有侧重"),
    body: t("左侧 8 个 tab：对话 / 概览 / 成本 / 安全 / 资产 / 知识库 / 活动 / 项目。⌘1..⌘8 一键切换。"),
    hint: t("活动页是热力图 + 24h 柱状图，看你最常在哪个时段用 AI。"),
  },
  {
    icon: "library",
    title: t("知识提取：把对话变资产"),
    body: t("打开任意会话，按 ⌘K 选「提取知识」会抽出摘要 / 决策 / TODO / 错误 / 命令 / 文件 6 类，自动进知识库。"),
    hint: t("知识库支持跨会话引用 —— 在 A 会话提取的结论，能在 B 会话被自动找到。"),
  },
  {
    icon: "shield",
    title: t("安全审计 + 一键处理"),
    body: t("安全 tab 会扫描所有会话里的风险操作（删库命令 / 凭据泄露 / 危险路径写入等），可勾选后批量「忽略」或「标记误报」。"),
    hint: t("支持导出 / 导入策略规则，换台机器也能继承。"),
  },
  {
    icon: "settings",
    title: t("设置：偏好、备份、关于"),
    body: t("右上角齿轮进设置：显示偏好（数字格式 / 货币 / 日期）、自动同步间隔、保留策略、预算超支通知、About + 更新日志。"),
    hint: t("「设置 → 数据」里能一键定位到加密备份目录，也可以导入 / 导出偏好 JSON。"),
  },
];
...
const STEPS_EXISTING = (): Step[] => [
  {
    icon: "chart",
    title: t("回顾你的历史 / 试试搜索 ⌘F"),
    body: t("库中已有对话了。先按 ⌘F 焦点搜索框（macOS）/ Ctrl+F（其它）找一条试试，或点开「概览」看整体趋势。"),
    hint: t("试试按来源筛选 chip：Cursor / Claude Code / ZCode / Codex 单独看。"),
  },
  {
    icon: "command",
    title: t("命令面板：⌘K 唤起"),
      ...
      >
        <div className="settings-header">
          <h2>
            ...
            {effectiveMode === "existing" ? t("欢迎回到 Threadock") : t("欢迎使用 Threadock")}
          </h2>
          <button className="settings-close" onClick={finish} aria-label={t("跳过引导")}><Icon name="close" size={14} /></button>
        </div>
