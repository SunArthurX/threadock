...
fn main() {
    ...
    let cmds: Vec<(&str, &str, &str)> = vec![
        ...
        ("sources_new_count", "-", "{zcode, claude_code, cursor, minimax, codex, total}"),
        ("ops_month_projection", "-", "MonthProjection"),
        ("ops_cache_trend", "days?", "CacheTrendRow[]"),
        ("audit_scan_conversation", "conversationId", "AuditFinding[]"),
        ("audit_finding_set_state", "fingerprint, status, note?", "void"),
        ("audit_finding_restore", "fingerprint", "void"),
        ("audit_finding_states", "-", "AuditFindingState[]"),
        ("governance_log_list", "limit?", "GovernanceLogRow[]"),
        ("storage_stats", "-", "{db_bytes, raw_count, raw_bytes, index_bytes}"),
