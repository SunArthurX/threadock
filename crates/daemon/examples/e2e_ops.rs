...
fn main() {
    ...
    for days in [Some(30i64), Some(7), None] {
        ...
        let ov = repo.ops_overview(days).unwrap_or_else(|e| panic!("overview({label}) 失败: {e}"));
        ...
        assert!(!bp.is_empty() && !bm.is_empty() && !tt.is_empty(), "聚合({label}) 不应为空");
    }
    println!("── 模型明细样本:");
    for m in repo.ops_by_model(Some(30)).unwrap().iter().take(6) {
        println!("   {} [{}] in={} out={} err={}", m.model, m.provider_id, m.input_tokens, m.output_tokens, m.errors);
    }
    println!("── 工具 Top3:");
    for t in repo.ops_tool_toplist(Some(30), 3).unwrap() {
        println!("   {} calls={} destructive={}", t.tool_name, t.calls, t.destructive);
    }
    drop(repo);
    ...
    println!("── 审计: {:?} 扫描 {} 消息 / {} 命令 → 高危 {} 中危 {} 低危 {}（共 {} 条发现）",
        t.elapsed(), report.scanned_messages, report.scanned_tool_calls,
        report.high, report.medium, report.low, report.findings.len());
