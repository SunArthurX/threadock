...
fn main() {
    ...
    // 1. 采集（4 源）
    let t = Instant::now();
    let (mut u_all, mut t_all) = ch_ops_metrics::collect_zcode(format!("{home}/.zcode/cli/db/db.sqlite")).unwrap();
    println!("zcode: usage={} tools={}", u_all.len(), t_all.len());
    let u = ch_ops_metrics::collect_minimax(format!("{home}/.minimax/v2/sqlite/runtime-state.sqlite")).unwrap();
    println!("minimax: usage={}", u.len()); u_all.extend(u);
    let (u, tc) = ch_ops_metrics::collect_claude_code(format!("{home}/.claude")).unwrap();
    println!("claude: usage={} tools={}", u.len(), tc.len()); u_all.extend(u); t_all.extend(tc);
    let (u, tc) = ch_ops_metrics::collect_codex(format!("{home}/.codex")).unwrap();
    println!("codex: usage={} tools={}", u.len(), tc.len()); u_all.extend(u); t_all.extend(tc);
    println!("── 采集总计: {:?}  usage={} tools={}", t.elapsed(), u_all.len(), t_all.len());
    ...
    let fallback = [("prov_zcode","zcode"),("prov_minimax-code","minimax-m3"),("prov_claude-code","claude"),("prov_codex","codex")];
    let mut cost_total = 0f64;
    for (model, pid, i, o) in repo.ops_model_token_totals().unwrap() {
        let m = model.to_lowercase();
        let key = ["glm-5.2","minimax-m3","codex","claude","zcode"].iter().find(|k| m.contains(**k)).map(|k| k.to_string())
            .or_else(|| fallback.iter().find(|(p,_)| *p==pid).map(|(_,k)| k.to_string()));
        if let Some(k) = key {
            let v = &pricing[&k];
    ...
    println!("── Overview: 请求={} tokens={} 成本=${:.2} 危险={} 工具={}", ov.total_requests, ov.total_tokens, ov.cost_usd, ov.destructive_calls, ov.total_tool_calls);
    for p in repo.ops_by_provider(None).unwrap() {
        println!("   {:14} req={:>6} tokens={:>14} err={}", p.provider, p.requests, p.total_tokens, p.errors);
    }
    let ts = repo.ops_timeseries_daily(Some(7)).unwrap();
    println!("── 近7天趋势: {} 天", ts.len());
    for d in ts.iter() { println!("   {} tokens={}", d.day, d.total_tokens); }
    drop(repo);
