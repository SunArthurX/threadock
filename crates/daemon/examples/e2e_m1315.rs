...
fn main() {
    ...
    for b in repo.ops_agent_benchmark(Some(30)).unwrap() {
        println!("   {:14} req={:>6} tokens={:>12} cost=${:>7.2} 成功={:.1}% 缓存={:.1}% 会话={}",
            b.provider, b.total_requests, b.total_tokens, b.cost_usd, b.success_rate, b.cache_hit_rate, b.sessions);
    }

    println!("── M14 周报数据:");
    let s = repo.ops_weekly_summary().unwrap();
    println!("   7天: 请求={} tokens={} 成本=${:.2} 浪费会话={}",
        s.overview.total_requests, s.overview.total_tokens, s.overview.cost_usd, s.waste_sessions);
    println!("   Agent 数: {} 健康数据: {} 条", s.benchmark.len(), s.health.len());
    ...
    if let Some(conv) = convs.first() {
        ...
        println!("── M15 时间线: 「{}」消息={} 事件={}",
            conv.effective_title().chars().take(30).collect::<String>(), msgs.len(), evts.len());
    }
