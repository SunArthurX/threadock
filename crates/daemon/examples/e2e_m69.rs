...
fn main() {
    ...
    let by_agent: Vec<(String, usize)> = {
        ...
        for a in &assets { *m.entry(a.provider.as_str().to_string()).or_insert(0) += 1; }
        m.into_iter().collect()
    };
    println!("M6 资产: {:?} 采集 {} 条 → {:?}", t.elapsed(), assets.len(), by_agent);
    for p in ["zcode", "codex", "claude-code", "minimax-code"] {
        let subset: Vec<_> = assets.iter().filter(|a| a.provider.as_str() == p).cloned().collect();
    ...
    println!("   入库 {} 条，含危险模式 {} 条；样本: {:?} v{} [{}]",
        listed.len(), risky,
        listed.iter().find(|a| a.kind == "plugin" && a.version.is_some()).map(|a| a.name.clone()).unwrap_or_default(),
        listed.iter().find(|a| a.kind == "plugin" && a.version.is_some()).and_then(|a| a.version.clone()).unwrap_or_default(),
        listed.first().map(|a| a.provider.clone()).unwrap_or_default());
    ...
    for p in ["zcode", "codex", "minimax-code"] {
        let subset: Vec<_> = autos.iter().filter(|a| a.provider.as_str() == p).cloned().collect();
        let _ = repo.replace_provider_automations(&format!("prov_{p}"), &subset);
    }
    for a in repo.list_automations().unwrap().iter().take(5) {
        println!("   [{}] {} {} {} {}", a.provider, a.name, a.kind, a.schedule.as_deref().unwrap_or(""), a.status.as_deref().unwrap_or(""));
    }
    ...
    println!("M7 前置 zcode usage 重灌（带 source_dir）: {:?} {} 条", t.elapsed(), zu.len());
    println!("── 按项目成本 Top5:");
    for d in repo.ops_cost_by_dir(None, 5).unwrap() {
        println!("   {} tokens={} cost=${:.2} req={}", d.dir, d.tokens, d.cost_usd, d.requests);
    }
    println!("── 缓存命中率:");
    for c in repo.ops_cache_stats(None).unwrap() {
        println!("   {:14} cache={}/{} 命中 {:.1}%", c.provider, c.cache_read_tokens, c.input_tokens + c.cache_read_tokens, c.hit_rate * 100.0);
    }

    // ── M9 异常 ──
    let an = repo.ops_anomalies(None).unwrap();
    println!("M9 异常: {} 条", an.len());
    for a in an.iter().take(8) { println!("   [{}] {} {}", a.severity, a.kind, a.detail); }
