...
### Added
- **中英文双语界面（设置 → 外观 → 语言）**：新增轻量 i18n（中文原文为
  key，英文查词典、缺失回退中文），全量替换 930 条界面文案；语言偏好
  localStorage 持久化（ch-pref-lang），切换即时生效（根组件 key 重挂载）；
  模块级常量（导航/选项卡/快捷键表等）函数化避免切换后不刷新；新增词典
...
（2026-08-23 重设 tag 并入搜索修复：结果按消息时间倒序 + 常见词低频命中会话不再被截断 + 索引 schema 自动迁移。）
...
- **搜索历史单条删除**：历史下拉每行 hover 出现 ×（保存的搜索同款交互），
  ...
  截断、最高 340px 内滚动，与下方会话列表拉开视觉层次
- **左栏会话列表横向滚动**：列表列较窄时长标题不再省略号截断——行区
  （`.list-virtual-container`）支持左右滑动（原生横向滚动条 + macOS
  shift+滚轮 / 触控板横滑），表头与工具栏固定不随动；虚拟行 `max-content`
  宽度由最长行撑开
- **顶栏搜索框一键清空**：输入有内容时框内出现 ✕（Esc 同效）——清关键词
  并退出搜索模式（原搜索结果态的「清除」按钮并入，交互统一）；清空后焦点
  保持在输入框
- **对话视图触控板横滑切会话（右栏详情）**：双指左滑 → 下一
  会话、右滑 → 上一会话（与 j/k 同一条 `navigateConv` 通路，加载详情 +
  列表滚动到可见）。纯检测器 `swipeStep`（横向意图门控 / 累积阈值 110 /
  冷却 700ms / 停顿重置 / 微抖过滤）+ 横向溢出让位（payload JSON 块等
  可横向滚动内容不劫持）+ 12 项矩阵测试；左栏列表自身支持横向滚动
  （见上），不参与横滑导航
- **会话甘特图（活动页）**：新增「会话甘特图」卡片——每会话一行，条 =
  `started_at → updated_at` 时间跨度，按 Agent 着色；卡片自带时间范围选择
  （7 天 / 30 天 / 90 天 / 1 年，**默认近 30 天**），独立于活动页全局范围
  （`list_conversations_by_date` 拉主任务会话）；悬停浮动详情（标题 / 来源 /
  起止 / 跨度）、点击跳转会话详情、「今天」刻度线（每分钟自动换位）、
  超 80 条截断提示。纯布局函数 `buildGanttRows`（窗口裁剪 / 倒序 /
  极短保底可见 / maxRows 截断）+ 14 项矩阵与交互测试
