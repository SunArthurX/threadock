...
- **会话甘特图（活动页）**：新增「会话甘特图」卡片——每会话一行，条 =
  `started_at → updated_at` 时间跨度，按 Agent 着色；跟活动页 90/180/365 天
  与年度范围联动（`list_conversations_by_date` 拉主任务会话）；悬停浮动
  详情（标题 / 来源 / 起止 / 跨度）、点击跳转会话详情、「今天」刻度线
  （每分钟自动换位）、超 80 条截断提示。纯布局函数 `buildGanttRows`
  （窗口裁剪 / 倒序 / 极短保底可见 / maxRows 截断）+ 13 项矩阵测试
